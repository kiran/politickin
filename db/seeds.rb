...
open("db/seeds/opencongress_senators.tsv") do |groups|
...
# run through opencongress reps data
open("db/seeds/opencongress_reps.tsv") do |groups|
  groups.read.each_line do |group|
    next if group.empty?
    fname, lname, json_string = group.chomp.split("\t")
    json = JSON.parse(json_string)
