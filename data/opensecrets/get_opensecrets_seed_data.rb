...
array_of_hashes.each do |congressman|
  ...
  name = congressman['firstname'] + " " + congressman['lastname']
  contrib_url = "#{URL}?method=candContrib&output=json&cid=#{congressman['crp_id']}&apikey=ffad49d015c6316def251af6c9cf595b"
  industry_url = "#{URL}?method=candIndustry&output=json&cid=#{congressman['crp_id']}&apikey=ffad49d015c6316def251af6c9cf595b"
  begin
    begin
    ...
    contrib_data = [congressman['firstname'], congressman['lastname'],
        congressman['crp_id'], {'opensecrets_contributors' => contrib_info.map {|el| el['@attributes'] }.gsub("=>", ":") }]
    industry_data = [congressman['firstname'], congressman['lastname'],
        congressman['crp_id'], {'opensecrets_industries' => industry_info.map {|el| el['@attributes'] }.gsub("=>", ":") }]
